...
- **Main checkout** → the canonical instance: session `mt-dev`, backend `3001`,
  frontend `5173`, logs `/tmp/mt-backend.log` + `/tmp/mt-frontend.log`. Running
  it here tears down and takes over `3001`/`5173` as before.
- **A worktree** → its own instance keyed by the worktree directory name:
  session `mt-dev-<slug>`, ports **deterministically derived** from the slug
  (backend `3001+offset`, frontend `5173+offset`), logs `/tmp/mt-<slug>-*.log`.
...
way. Step 1 writes that resolver to `/tmp/mt-dev-resolve.sh`; **every later
...
> /tmp/mt-dev-resolve.sh` to recompute `ROOT`, `SLUG`, `BACKEND_PORT`,
...
1. Resolve this checkout's instance identity, then tear down **only that
   ...
   `3001`/`5173`):
   ```bash
   cat > /tmp/mt-dev-resolve.sh <<'RESOLVE'
   ...
   if [ "$ROOT" = "$MAIN" ]; then
     ...
     FRONTEND_PORT=5173
     SESSION=mt-dev
     BACK_LOG=/tmp/mt-backend.log
     FRONT_LOG=/tmp/mt-frontend.log
   else
     ...
     SESSION="mt-dev-$SLUG"
     BACK_LOG="/tmp/mt-$SLUG-backend.log"
     FRONT_LOG="/tmp/mt-$SLUG-frontend.log"
   fi
   RESOLVE
   source /tmp/mt-dev-resolve.sh
...
Because each instance is its own `mt-dev[-<slug>]` session with its own log
files, you can enumerate everything that is up without disturbing it:
```bash
tmux ls 2>/dev/null | grep -E '^mt-dev'
# and the URL each frontend bound to:
for f in /tmp/mt-frontend.log /tmp/mt-*-frontend.log; do
