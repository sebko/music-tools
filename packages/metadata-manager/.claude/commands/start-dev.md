---
description: Start metadata-manager backend and frontend dev servers in a detached tmux session
---

Start both metadata-manager dev servers inside a **detached tmux session** so they
...
2. Start the detached session with the backend window. `tee` keeps the live
   ...
   it to the log file:
   ```bash
   tmux new-session -d -s mm-dev -n backend -c /Users/sebastiankey/github/dj-tools/packages/metadata-manager/backend
...
3. Add the frontend window (enforce port 5174, fail loudly if taken):
   ```bash
   tmux new-window -t mm-dev -n frontend -c /Users/sebastiankey/github/dj-tools/packages/metadata-manager/frontend
