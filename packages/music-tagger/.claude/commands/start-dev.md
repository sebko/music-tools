---
description: Start music-tagger backend and frontend dev servers in a detached tmux session
---

Start both music-tagger dev servers inside a **detached tmux session** so they
...
2. Preflight the target checkout — a fresh worktree starts with no
   ...
   echo "Serving music-tagger from: $ROOT"
   ...
   npm --prefix "$ROOT/packages/music-tagger/backend" exec prisma generate
   ...
   if [ "$MAIN" != "$ROOT" ]; then
     for f in .env .env.test; do
       SRC="$MAIN/packages/music-tagger/backend/$f"
       DST="$ROOT/packages/music-tagger/backend/$f"
...
3. Start the detached session with the backend window. `tee` keeps the live
   ...
   ```bash
   ROOT=$(git rev-parse --show-toplevel)
   tmux new-session -d -s mt-dev -n backend -c "$ROOT/packages/music-tagger/backend"
...
4. Add the frontend window (enforce port 5173, fail loudly if taken):
   ```bash
   ROOT=$(git rev-parse --show-toplevel)
   tmux new-window -t mt-dev -n frontend -c "$ROOT/packages/music-tagger/frontend"
